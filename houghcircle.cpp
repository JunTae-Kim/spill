...
//	softServoSetup(SERVO, -1, -1, -1, -1, -1, -1, -1);
	...
	while (1) {
		...
		Mat element1 = getStructuringElement(element_shape, Size(2,2));
		...
		//morphologyEx(edgeimg, erimg, MORPH_CLOSE, element2);
		//dilate(edgeimg, erimg, element1);
		...
		HoughCircles(edgeimg, circles, CV_HOUGH_GRADIENT, 3, 3, 300, 50, 0, 20);
		...
		else if (leftP2.x >= 0 && leftP2.x < 30) {
			value = 2;
		}
		else if (leftP2.x >= 30 && leftP2.x <= 200) {
			value = 3;
		}
		else if (rightP2.x >= 120 && rightP2.x < 260) {
			value = -3;
		}
		else if (rightP2.x >= 260 && rightP2.x < 290) {
			value = -2;
		}
